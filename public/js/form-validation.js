...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nurseForm) {
        nurseForm.addEventListener('submit', function(e) {
            const isDraft = new FormData(this).get('action') === 'draft';
            const button = isDraft ? draftBtn : submitBtn;
            const buttonText = isDraft ? draftBtnText : submitBtnText;
            const statusDiv = isDraft ? draftStatus : submitStatus;

            if (button && buttonText && statusDiv) {
                // Show loading state
                button.disabled = true;
                buttonText.textContent = isDraft ? 'Saving Draft...' : 'Submitting...';
                statusDiv.textContent = isDraft ? 'Saving assessment as draft, please wait...' : 'Submitting assessment, please wait...';

                // Announce loading state
                announceToScreenReader(isDraft ? 'Saving assessment as draft' : 'Submitting assessment', 'assertive');
            }
        });
    }
    ...
    function initializeMobileAccessibility() {
        ...
        if (isMobile) {
            ...
            announceToScreenReader('Mobile device detected. Touch-friendly interface enabled.', 'polite');
        }
        ...
        if (navigator.userAgent.includes('Mobile') || navigator.userAgent.includes('Android')) {
            // Add screen reader optimizations for mobile
            const srOnlyElements = document.querySelectorAll('.sr-only');
            srOnlyElements.forEach(element => {
                element.setAttribute('aria-live', 'polite');
            });

            // Improve heading hierarchy for mobile screen readers
        ...
        window.addEventListener('orientationchange', function() {
            setTimeout(() => {
                ...
                const orientationText = Math.abs(orientation) === 90 ? 'landscape' : 'portrait';
                announceToScreenReader(`Device rotated to ${orientationText} orientation`, 'polite');

                // Re-adjust mobile layout if needed
                adjustForTablet();
            }, 500);
        });

        // Prevent text selection on UI elements for better touch experience
        const uiElements = document.querySelectorAll('button, .btn, .navbar, .card-header');
